function [] = visualise_results(obj_struct,vars_struct,name,iso_params,forcefields)
...
for k = 1:5
    obj_data = obj(:,k);
    makeplot(obj_data, c, FF,forcefields)
    nexttile;
end
...
for k = forcefields
    ...
    if k<length(FF)/2 + 1
        try
            scatter(obj{k,iter}(:,2),obj{k,iter}(:,1),15,'s','MarkerEdgeColor',c(k,:),'MarkerFaceColor',c(k,:),'DisplayName',FF{k})
        catch
        ...
        try
            scatter(obj{k,iter}(:,2),obj{k,iter}(:,1),15,'MarkerEdgeColor',c(k-(length(FF)/2),:),'MarkerFaceColor',c(k-(length(FF)/2),:),'DisplayName',FF{k})
        catch
            scatter([0 0],[0 0],15,'MarkerEdgeColor',c(k-(length(FF)/2),:),'MarkerFaceColor',c(k-(length(FF)/2),:),'DisplayName',FF{k})
        end
    end
end
xlabel('Pr [$mol_{CO_2}\ ton^{-1} s^{-1}$]') ;
ylabel('En [$kWh\ ton_{CO_2}^{-1}$]') ;
...
print(sprintf('%s/visualisations/%s',pwd,name),'-dpng','-r300')
    ...
    function []= makeplot(obj,c,FF, forcefields)
    hold on;
    for j = forcefields
        if j<length(FF)/2 + 1
            try
                scatter(obj{j,1}(:,2),obj{j,1}(:,1),15,'MarkerEdgeColor',c(j,:),'MarkerFaceColor',c(j,:),'DisplayName',FF{j})
            catch
            ...
            try
                scatter(obj{j,1}(:,2),obj{j,1}(:,1),15,'s','MarkerEdgeColor',c(j-(length(FF)/2),:),'MarkerFaceColor',c(j-(length(FF)/2),:),'DisplayName',FF{j})
            catch
