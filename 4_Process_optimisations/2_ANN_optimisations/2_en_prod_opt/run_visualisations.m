% visualise the outputs for the En-Pr Pareto optimisations across every
% iteration
clear all; close all; clc;

addpath('opt_results\'); 
myFolder = sprintf('%s/opt_results/',pwd);
opt_results_path = fullfile(myFolder, '*_objectivesEcon.mat');
var_results_path = fullfile(myFolder, '*_variablesEcon.mat');
...
% Discard the neutral FF index
forcefields = [1,2,4,5,6,8];
...
for k = 1:length(optFiles)
    ...
    visualise_results(opt_data,var_data,name,iso_params,forcefields);
end
